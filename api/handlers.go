...
// LstmHandler agrupa os handlers da API para LSTM
type LstmHandler struct {
...
// NewLstmHandler cria um novo handler com o modelo LSTM
func NewLstmHandler(m *model.LstmModel) *LstmHandler {
	return &LstmHandler{model: m}
}
...
// RegisterRoutes registra todas as rotas da API para LSTM
func (h *LstmHandler) RegisterRoutes(mux *http.ServeMux) {
	// Rotas da API
	mux.HandleFunc("/api/ask", h.handleAsk)
	mux.HandleFunc("/api/health", h.handleHealth)
...
// handleAsk handler para geração de texto LSTM
func (h *LstmHandler) handleAsk(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Access-Control-Allow-Origin", "*")
	w.Header().Set("Access-Control-Allow-Methods", "GET, POST, OPTIONS")
	w.Header().Set("Access-Control-Allow-Headers", "Content-Type")
...
// handleHealth handler para health check LSTM
func (h *LstmHandler) handleHealth(w http.ResponseWriter, r *http.Request) {
	h.sendJSON(w, map[string]string{
		"status": "ok",
		"model":  "LMCS LSTM",
	}, http.StatusOK)
...
// sendJSON helper para enviar respostas JSON LSTM
func (h *LstmHandler) sendJSON(w http.ResponseWriter, data interface{}, status int) {
	w.WriteHeader(status)
	if err := json.NewEncoder(w).Encode(data); err != nil {
		log.Printf("Erro ao codificar resposta JSON: %v", err)
	}
...
// sendError helper para enviar respostas de erro LSTM
func (h *LstmHandler) sendError(w http.ResponseWriter, message string, status int) {
	h.sendJSON(w, ErrorResponse{
		Success: false,
		Error:   message,
	}, status)
